...
namespace Haply.hAPI.Samples
{
    public class HelloWall : MonoBehaviour
    {
        ...
        private void SetInitialVelocity()   {
            ...
            m_Moon.GetComponent<Rigidbody2D>().velocity += (Vector2)m_Moon.transform.right * Mathf.Sqrt((G * mass_earth) / r);
        }

        private void Gravity()  {
            Debug.Log("Computing Gravity...\n");
            float r = Vector2.Distance(m_Earth.transform.position, m_Moon.transform.position);
            m_EarthForce = (m_Earth.transform.position - m_Moon.transform.position).normalized * (G * (mass_earth * mass_moon) / (r * r));
